...
class PolygonEquityData:
    ...
    def __init__(self, api_key, ticker_list, catalog_path):
        """
        Args:
            api_key (module): api_key containing polygon api key
        ...
        self.__timeframes = {
            BarAggregation.MINUTE: Timespan.MINUTE,
            BarAggregation.HOUR: Timespan.HOUR,
            BarAggregation.DAY: Timespan.DAY,
            BarAggregation.WEEK: Timespan.WEEK,
        }
    ...
    def get_tickers(self):
        ...
        ref_client = ReferenceClient(self.api_key, use_async=False)
    ...
    def get_bar_data_for_tickers(self, start_date, end_date, multiplier: int, timespan: BarAggregation):
        ...
        stocks_client = StocksClient(
            self.api_key, use_async=False, read_timeout=120
        )
        ...
        for ticker in self.tickers:
            ...
            ticker_data = self.get_bar_data_for_single_ticker(
                stocks_client, ticker["ticker"], start_date, end_date, multiplier, self.__timeframes.get(timespan)
            )
            ...
            bar_type = BarType(
                instrument.id,
                BarSpecification(multiplier, timespan, PriceType.LAST),
    ...
    def get_bar_data_for_single_ticker(
            self, stocks_client, ticker, start_date, end_date, multiplier=1, timespan=Timespan.DAY):
